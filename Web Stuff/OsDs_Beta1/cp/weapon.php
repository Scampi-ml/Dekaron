	...
	if(empty($_POST['select'])) {
		echo "<center><br><form action='?function=weapon&uc=".$uc."' method='POST'>
			<table class='innertab'>
				<tr>
					<td colspan='2' align='center'><b><u>Send weapon to character</b></u></td>
				</tr>
				<tr>
				...
				<tr>
					<td align='center' colspan='2'>
						...
						<input type='submit' value='Select weapon'>
					</td>
				</tr>
			</table>
		</form></center>";
		...
		if($count1 < '1') {
			...
			echo "<br>There were several characters with the same name found.<br>Please check this name in the database.";
		} else {
				...
				<table class='innertab'>
					...
					<tr>
						<td align='center' colspan='2'>
							...
							<input type='submit' value='Show all available weapons'>
						</td>
					</tr>
				</table>
			</form>
		...
		echo "<center><br><form action='?function=weapon&uc=".$uc."' method='POST'>
			<table class='innertab'>
				...
				<tr>
					<td align='center'>Available Weapons</td>
				...
				<tr>
					<td align='center'>On what should be the weapon +</td>
				...
				<tr>
					<td align='center' colspan='2'>
						...
						<input type='hidden' name='charname' value='".$_POST['charname']."'>
						<input type='submit' value='Send weapon to character'>
					</td>
				</tr>
			</table>
		</form></center>";
		...
		if(empty($row1[0]) || $row1[0] == '0') {
			echo "<br>Either there are the desired weapon with the + is not required or there is an error in the database.";
		} else {

			$ms_con = mssql_connect($mssql['host'],$mssql['user'],$mssql['pass']);
			mssql_query("EXEC character.dbo.SP_POST_SEND_OP '".$_POST['char_no']."','".$gm_name."',1,'A weapon for you','Have fun with this weapon.','".$row1[0]."',0,0",$ms_con);

			echo "<br>The weapon mail has been sent successfully.";
