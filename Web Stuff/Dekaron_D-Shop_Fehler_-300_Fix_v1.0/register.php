...
if($_POST['activ'] == '1') {
		...
		if(empty($_POST['accname']) || empty($_POST['accpass1']) || empty($_POST['accpass2']) || empty($_POST['accquest']) || empty($_POST['accname']) || empty($_POST['accansw']) || empty($_POST['accmail']) || empty($_POST['result1'])) {
			...
			$result6 = mssql_query("SELECT user_no FROM account.dbo.Tbl_user ORDER BY user_no Desc'",$con); 
			$row6 = mssql_fetch_row($result6);
			...
			if($row6[0] % '2') {
				...
				$fake_name = $_POST['accname']."_fake";
				mssql_query("INSERT INTO account.dbo.USER_PROFILE (user_id,user_pwd,resident_no,user_type,login_flag,login_tag,ipt_time,login_time,logout_time,user_ip_addr,server_id) VALUES ('".$fake_name."','".$accpass."','801011000000','1','0','Y','".$date."',null,null,null,'000')",$con);
				mssql_query("INSERT INTO account.dbo.Tbl_user (user_id,user_pwd,user_mail,user_answer,user_question) VALUES ('".$fake_name."','".$_POST['accpass1']."','".$_POST['accmail']."','".$_POST['accquest']."','".$_POST['accansw']."')",$con);
			...
			mssql_query("DELETE FROM account.dbo.Tbl_user WHERE user_id LIKE '%_fake%'",$con);
			mssql_query("DELETE FROM account.dbo.USER_PROFILE WHERE user_id LIKE '%_fake%'",$con);
	...
	$con = mssql_connect($mssql['host'],$mssql['user'],$mssql['pass']) or die($language['mssql_e1']);
	$result1 = mssql_query("SELECT * FROM account.dbo.USER_PROFILE WHERE user_id NOT LIKE '%_fake%'",$con);
	$row1 = mssql_num_rows($result1);
