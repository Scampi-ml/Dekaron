...
else
{
 ...
 $query = $db_character->query("SELECT wRetMapIndex,wRetPosY,wRetPosX,character_no,character_name FROM user_character WHERE character_no = '".$_GET['character']."' ");
$getCharacterPos = $db_character->fetchArray($query);

$map = $getCharacterPos['wRetMapIndex'];
$x = $getCharacterPos['wRetPosY'];
$y = $getCharacterPos['wRetPosX'];
