...
echo '<div id="serverinfo">'.LAN_send.' '.LAN_armor.' '.LAN_to.': '.$GET_CHARACTER_NAME.'</div>';
...
$getCharacter = $db_character->fetchArray($query1);
			...
			<table >
				<tr class='even'>
					...
					<td ><select name='min_max' style='width:213px;'>
						...
						<br>
						<small>".LAN_current." ".LAN_level.": ".$getCharacter['wLevel']."</small>
					</td>
				</tr>
				<tr class='even'>
					<td >".LAN_type."</td>
					<td ><select name='item_type' style='width:213px;'>
							<option value='1'>".LAN_armor."</option>
							<option value='0'>".LAN_helmet."</option>
							<option value='2'>".LAN_pants."</option>
							<option value='3'>".LAN_boots."</option>
							<option value='4'>".LAN_gloves."</option>
						</select>
					</td>
				</tr>
				<tr class='even'>
	...
	$result1 = $db_osds->query(("SELECT * FROM itemarmor WHERE ReqLv >= '".$minlv."' AND ReqLv <= '".$maxlv."' AND Name NOT LIKE '%+%' AND Code = '".$_GET['item_type']."' ORDER BY Convert(int, ReqLv) ASC ");
	...
	<legend>".LAN_step." 2</legend>
		<table >
			...
			<tr class='even'>
				...
				<td><input name='post_title' size='50' type='text' value='".LAN_sa_def_subject."'></td>
			</tr>
			<tr class='even'>
				<td><label>".LAN_message."</label></td>
				<td><textarea name='body_text' cols='45' rows='5' style='height: 100px;'>".LAN_sa_def_message."</textarea></td>
			</tr>
...
elseif($_GET['select'] == '2')
{
	if($_GET['how_plus'] == '0')
	{
		$result1 = $db_osds->query("SELECT * FROM itemarmor WHERE Name = '".$_GET['item_name']."'");
	}
	else
	{
		$result1 = $db_osds->query("SELECT * FROM itemarmor WHERE Name = '".$_GET['item_name']." +".$_GET['how_plus']."'");
	}
	...
	else
	{
		$query1 = $db_character->query("EXEC SP_POST_SEND_OP '".$_GET['character']."','".$_SESSION['USER']."',1,'".$_GET['post_title']."','".$_GET['body_text']."','".$row1['Id']."',0,0");
			echo '<div class="success msg">'.LAN_armor.' '.LAN_send.'</div>';
