...
if($is_gm != '0') {
	if(empty($_POST['select'])) {
		echo "<center><br><form action='?do=account&id=".$id."' method='POST'>
			<table class='innertab'>
				<tr>
					<td colspan='2' align='center'><img src='images/content/content_modify_acc.png' align='left'><br></td>
				</tr>
				<tr>
		...
		if(empty($_POST['charname']) && empty($_POST['accname'])) {
			echo "<br>You must input at least a character name or account name below.<br><a href='javascript:history.back()'>Go Back</a>";
		} elseif(!empty($_POST['charname']) && !empty($_POST['accname'])) {
			echo "<br>You may only enter a character name or account name below.<br><a href='javascript:history.back()'>Go Back</a>";
		} else {
			...
			if(empty($_POST['charname']) && $count1 < '1') {
				echo "<br>Could not find a account with the names.<br><a href='javascript:history.back()'>Go Back</a>";
			} elseif(empty($_POST['charname']) && $count1 > '1') {
				echo "<br>The specified account name exists in the database several times.<br><a href='javascript:history.back()'>Go Back</a>";
			} elseif(empty($_POST['accname']) && $count1 < '1') {
				echo "<br>Could not update the character name.<br><a href='javascript:history.back()'>Go Back</a>";
			} elseif(empty($_POST['accname']) && $count1 > '1') {
				echo "<br>The specified character name exists several times in the database.<br><a href='javascript:history.back()'>Go Back</a>";
			} elseif(empty($_POST['accname']) && $count2 < '1') {
				echo "<br>The account on the specified character is not found.<br><a href='javascript:history.back()'>Go Back</a>";
			} elseif(empty($_POST['accname']) && $count2 > '1') {
				echo "<br>The account on which this character is running several times.<br><a href='javascript:history.back()'>Go Back</a>";
			} else {
				...
				echo "<center><form action='?do=account&id=".$id."' method='POST'>
					<table class='innertab'>
						<tr>
							<td colspan='3' align='center'><br><img src='images/content/content_modify_acc.png' align='left'></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td align='center'>Current Stats</td>
							<td align='center'>New Stats</td>
						</tr>
						<tr>
							<td align='center'>Account Name</td>
		...
		if(empty($_POST['user_pwd']) || empty($_POST['user_mail']) || empty($_POST['user_ques']) || empty($_POST['user_answ'])) {
			echo "<br>You did not filled in all fields.<br><a href='javascript:history.back()'>Go Back</a>";
		} elseif(!preg_match("/[0-9a-zA-Z]?/", $_POST['user_pwd'])) {
			echo "<br>The password contains characters which are not allowed in the password.<br><a href='javascript:history.back()'>Go Back</a>";
		} elseif(strlen($_POST['user_pwd']) < 3 || strlen($_POST['user_pwd']) > 15) {
			echo "<br>The password must be at least 3 and not more than 15 characters long.<br><a href='javascript:history.back()'>Go Back</a>";
		} elseif(!preg_match("/^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/i",$_POST['user_mail'])) {
			echo "<br>This is not a valid email address.<br><a href='javascript:history.back()'>Go Back</a>";
		} elseif(strlen($_POST['user_ques']) < 3 || strlen($_POST['user_ques']) > 20) {
			echo "<br>The secret question must be at least 3 characters long and may not be longer than 20 characters.<br><a href='javascript:history.back()'>Go Back</a>";
		} elseif(strlen($_POST['user_answ']) < 3 || strlen($_POST['user_answ']) > 20) {
			echo "<br>The secret answer must be at least 3 characters long and may not be longer than 20 characters.<br><a href='javascript:history.back()'>Go Back</a>";
		} else {
			...
			echo "The account update was successful.";
		}
	} else {
