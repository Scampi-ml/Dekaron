var Router = {
	loadedJS: [],
	loadedCSS: [],
	first: true,
	page: false,
	initialize: function(){
		if(history.pushState){
			$("a[href*='" + Config.URL + "']").each(function(){
				if(!$(this).attr('data-hasEvent') && $(this).attr("target") != "_blank"){
					$(this).attr('data-hasEvent', '1');

					$(this).click(function(event){
						$("html").css("cursor", "wait");
						var href = $(this).attr("href");
						var direct = $(this).attr("direct");
						Router.load(href, direct);
						history.pushState('', 'New URL: ' + href, href);
						event.preventDefault();
					});
				}
			});
		}
	},
	load: function(link, direct){
		if(Router.first){
			Router.first = false;
			$(window).bind('popstate', function(){
				Router.load(location.pathname, 0);
			});
		}
		Router.page = link;
		if(/logout/.test(link)){
			window.location = link;
		}else if(/admin/.test(link)){
			window.location = link;
		}else if(direct == "1"){
			window.location = link;
		}else{
			$.get(link, { is_json_ajax: "1" }, function(data, textStatus, response){
				if(/^\<!DOCTYPE html\>/.test(data)){
					window.location.reload(true);
					return;
				}
				if(Router.page == link)
				{
					window.scrollTo(0, 0);
					try
					{
						data = JSON.parse(data);
					}
					catch(error)
					{
						data = {
							title: "Error",
							content: "ERROR<br /><br /><b>Error data:</b> " + data,
							js: null,
							css: null,
							slider: false,
						};
					}

					if(data.slider){
						$("#sliderContainer").show();
						$('#slider').nivoSlider({
						    effect: '' + Config.SliderEffect + '', 
						    animSpeed: 500,
						    pauseTime: Config.SliderPauseTime,
						    startSlide: 0, 
						    directionNav: false,
						    controlNav: false,
						    controlNavThumbs: false,
						    pauseOnHover: true,
						    manualAdvance: false,
						    randomStart: false 
						});
					}
					else
					{
						$("#sliderContainer").hide();
					}

					
					$("html").css("cursor", "default");
					$("#content_ajax").html(data.content);
					$("title").html(data.title);
					Router.initialize();
					if($.inArray(data.css, Router.loadedCSS) == -1 && data.css.length > 0){
						Router.loadedCSS.push(data.css);
						$("head").append('<link rel="stylesheet" type="text/css" href="' + Config.URL + 'application/' + data.css + '" />');
					}
					if($.inArray(data.js, Router.loadedJS) == -1 && data.js.length > 0){
						Router.loadedJS.push(data.js);
						require([Config.URL + "application/" + data.js]);
					}
				}
			}).fail(function(){
				if(Router.page == link){
					$("body").css("cursor", "default");
					$("title").html("DekaronCMS");
					UI.alert("Page Error! Trying to load the page... <center style='margin-top:20px;'><img src='" + Config.URL + "application/images/modal-ajax.gif' /></center>", 3000);
					setTimeout(function(){window.location = link;}, 3000);
				}
			});
		}
	}
}
$(document).ready(Router.initialize);