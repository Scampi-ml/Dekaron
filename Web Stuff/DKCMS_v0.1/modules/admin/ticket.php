...
if(isset($_SESSION['admin'])){
			...
			while($tickets = mssql_fetch_array($gettickets)){
				echo "
					<tr>
						...
						<td>
							<a href ='?dkcms=admin&amp;page=ticket&amp;a=$tickets[ticketid]&amp;ticket=Yes'>
								" . $tickets['title'] . "
							</a>
						</td>
						<td>
	...
	error_reporting(E_ALL);
		...
		$getResponse = mssql_query("SELECT * FROM dkcms.dbo.dkcms_tcomments WHERE ticketid = '".$_GET['a']."'");
		$countTicket = mssql_num_rows($getResponse);
		echo "
			<fieldset>
				<legend>
					Viewing Your Ticket
				...
				Responses: <br/><br/>
				";
				while($c = mssql_fetch_array($getResponse)){
					echo $c['user_name'] . " posted on " . $c['date_com'] . "<br/><br/> ".$c['content']."<hr/>";
				}
				if($countTicket < 1){
					echo "Please make a response to this ticket.";
				}
				echo "
					...
					<form method='post' action''>
						...
						<center>
							<input type='submit' name='subcomment' value='Submit Response'/>
				...
				if(isset($_POST['close'])){
					...
					if($closeTicket){
						...
						<meta http-equiv='refresh' content='2; url=?dkcms=admin&amp;page=ticket'/>
						";
					}
				}
			echo "
