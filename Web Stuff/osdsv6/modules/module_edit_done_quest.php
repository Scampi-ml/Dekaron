...
if (isset($_POST) && !empty($_POST))
{
	if(isset($_POST['quest']) && !empty($_POST['quest']))
	{
		if(is_array($_POST['quest']))
		{
			...
			{
				$SQLquery3 = $db->SQLquery("DELETE FROM character.dbo.User_Quest_Done WHERE character_no = '%s' AND q_index = '%s' ", $_POST['character'], $quest);
			}
		}
	}
	echo notice_message_admin('Done quests successfully deleted', '1', '0', 'index.php?get=module_edit_done_quest&character='.$_POST['character'].'');
}
else
{
	flush_this();
	$SQLquery1 = $db->SQLquery("SELECT * FROM character.dbo.User_Quest_Done WHERE character_no = '%s'", $_GET['character']);
...
<table width="95%" border="0" align="center" cellpadding="0" cellspacing="0"  class="table_panel" >
	...
	if ($qnum1 == '0')
	{
		echo '<tr><td align="center" class="panel_text_alt_list" colspan="3">No quests found</td></tr>';
	}
	else
	{
		flush_this();
	...
	?>
    <tr>
    	<td align="right" class="panel_buttons" colspan="3"><input type="submit" value="Delete Quest" onclick="ask_url('Are you sure you want to delete those quests?','index.php?get=module_edit_done_quest&character=<?php echo $_GET['character']; ?>')"></td>
    </tr>
</table>
