...
class FormValidator 
{
	...
	function check($variable,$validator,$error)
	{
    ...
    {
		$msg = '';
		foreach ($this->error_hash as $key=>$val)
		{
			$msg .= '<p class="msg_error">'.$val.'</p>';
		}
        return $msg;
    }
	...
	function ValidateCommand($command,$command_value,$input_value,&$default_error_message,$variable_name,$formvariables)
	{
		...
		switch($command)
		{
			...
			case 'alnum':
						{
							$bret= $this->test_datatype('/[^A-Za-z0-9]/',$input_value);
							if(false == $bret)
							{
            ...
            case 'numeric':
						{
							$bret= $this->test_datatype('/[^0-9]/',$input_value);
							if(false == $bret)
							{
			...
			case 'alpha':
						{
							$bret= $this->test_datatype('/[^A-Za-z]/',$input_value);
							if(false == $bret)
							{
