...
//查询用户名是否存在
    ...
    if ($_POST[level]<=$wLevel ) $errors[]="输入等级小于玩家现有等级";
    if ($_POST[level]>180 ) $errors[]="输入等级大于180级";
	if(!preg_match("/^[0-9]{1,3}$/i",$_POST[level])) $errors[]="等级只能为数字,最少为1位最大3位";
    if(sizeof($errors)>0){
       $errorStr .= "<br><font>";
       $errorStr .= "信息输入错误：";
    ...
    $level=$_POST[level]-$wLevel;
    $level_query="update character.dbo.user_character set wLevel=wLevel+$level,wStatPoint=wStatPoint+$level*5,wSkillPoint=wSkillPoint+$level where character_no='$character_no'";
