...
class API
{
	 ...
	 * @version 1.0
	 */
    protected function _call($method, $uri, $params = array(), $format = NULL, $return = true)
    {
    	if ($format !== NULL)
		{
			$this->format($format);
	...
	public function debug()
	{
		...
		{
			$output .= "<pre>".htmlentities($this->response_string)."</pre>\n\n";
		}
		...
		{
			$output .= "<h3>Errors</h3>";
			$output .= "<strong>Code:</strong> ".$this->_ci->curl->error_code."<br/>\n";
			$output .= "<strong>Message:</strong> ".$this->_ci->curl->error_string."<br/>\n";
			$output .= "=============================================<br/>\n";
		}
		...
		foreach($this->_ci->curl->info as $key=>$val)
		{
			$output .= '['.$key.'] => '.$val.'<br>';
		}
		...
		echo "window.open('debug.php?code=".urlencode(base64_encode($output))."','Debug','width=500,height=600,resizable,scrollbars=yes');";
		echo "</script>";			
