...
//		$SQL[] = "DROP TABLE ".ipsRegistry::dbFunctions()->getPrefix()."forums";
	...
	function step_18()
	{
		...
		$SQL[] = "INSERT INTO ".ipsRegistry::dbFunctions()->getPrefix()."upgrade_history (upgrade_version_id, upgrade_version_human, upgrade_date, upgrade_mid, upgrade_notes) VALUES ('".THIS_LONG_VERSION."', '".THIS_HUMAN_VERSION."', '0', '0', '');";
	...
	{
		$this->DB->cur_startrow = $a;
		return "SELECT TOP ".($a+$b)." m.*, me.id as mextra FROM ".ipsRegistry::dbFunctions()->getPrefix()."members m LEFT JOIN ".ipsRegistry::dbFunctions()->getPrefix()."member_extra me ON ( me.id=m.member_id )";
	}

	function sql_members_email( $a )
	{
		return "select TOP ".$a." email, count(email) as count from ".ipsRegistry::dbFunctions()->getPrefix()."members group by email order by count desc";
	}

	function sql_members_email_update( $push_auth )
	{
		return "UPDATE ".ipsRegistry::dbFunctions()->getPrefix()."members SET member_group_id=1, email=rtrim(cast(id as char))+'-'+email where id IN(".implode(",", $push_auth).")";
	}

	function sql_members_converge( $start, $end )
	{
		$this->setTableIdentityInsert( members_converge, 'ON' );
		return "SELECT m.*, c.converge_id as cid FROM ".ipsRegistry::dbFunctions()->getPrefix()."members m LEFT JOIN ".ipsRegistry::dbFunctions()->getPrefix()."members_converge c ON ( c.converge_id=m.member_id ) WHERE id >= $start and id < $end";
	}
}
