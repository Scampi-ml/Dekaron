 ...
 * Last Updated: $Date: 2010-12-17 07:40:13 -0500 (Fri, 17 Dec 2010) $
 * </pre>
 *
 * @author 		$author$
	...
	public function execute( $memberID, $words, $start=0, $end=50, $folders=array() )
	{
		...
		if ( $words )
		{
			...
			$this->DB->query( "SELECT SQL_CALC_FOUND_ROWS mt_id, mt_first_msg_id FROM ( ( SELECT t.mt_id, t.mt_first_msg_id
									...
									WHERE (t.mt_id=m.map_topic_id AND m.map_user_id=" . $memberID . " AND m.map_user_banned=0 AND t.mt_is_deleted=0) AND MATCH( t.mt_title ) AGAINST( '$words' IN BOOLEAN MODE )
									ORDER BY t.mt_last_post_time DESC )
								UNION
								( SELECT p.msg_topic_id, p.msg_id
									FROM {$dbpre}message_posts p, {$dbpre}message_topic_user_map m
									WHERE (p.msg_topic_id=m.map_topic_id AND m.map_user_id=" . $memberID . " AND m.map_user_banned=0 AND t.mt_is_deleted=0) AND MATCH( p.msg_post ) AGAINST( '$words' IN BOOLEAN MODE )
