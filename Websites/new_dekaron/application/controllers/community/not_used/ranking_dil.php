...
class Ranking_dil extends MY_Controller
{	
	...
	{
		$my_data = $this->cache->file->get('ranking_dil');
		if($my_data === FALSE)
		{		
			...
			$query = $this->db_character->query("
				SELECT TOP ".$ranking_limit." 
				  ...
				  SUM(user_character.dwMoney + user_character.dwStoreMoney + user_character.dwStorageMoney) AS total,
			...
			$this->cache->file->save('ranking_dil', $result, $this->config->item('ranking_update_interval'));
		}
		else
		{
			$result =  $my_data;
		...
		$this->load->view('ranks/view_ranking_dil', $this->template_data);
	...
	public function Force()
	{
		...
		else
		{
			...
			if($get_password == $this->config->item('ranking_force_update_password'))
			{
				...
				redirect('ranks/ranking_dil', 'refresh'); 					
			}
			else
			{
