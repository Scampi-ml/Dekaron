...
class Ranking_ip extends MY_Controller
{	
	...
	{
		$my_data = $this->cache->file->get('ranking_ip');
		if($my_data === FALSE)
		{		
			...
			$query = $this->db_character->query("
				...
				WHERE
				  (user_character.character_name NOT LIKE '[[]%]%' AND user_character.wChaoticLevel != '0')
				ORDER BY
				  wChaoticLevel DESC
			");
			$result = $query->result_array();
			$this->cache->file->save('ranking_ip', $result, $this->config->item('ranking_update_interval'));
		}
		else
		{
			$result =  $my_data;
		...
		$this->load->view('ranks/view_ranking_ip', $this->template_data);
	...
	public function Force()
	{
		...
		else
		{
			...
			if($get_password == $this->config->item('ranking_force_update_password'))
			{
				...
				redirect('ranks/ranking_ip', 'refresh'); 					
			}
			else
			{
