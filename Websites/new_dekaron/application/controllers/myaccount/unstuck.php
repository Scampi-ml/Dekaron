...
class Unstuck extends MY_Controller
{
	...
	public function DoUnstuck()
	{
		...
		{
			$ListCharacters = $this->m_character->ListCharacters();
			$this->template_data['template']['ListCharacters'] = $ListCharacters;
			...
			{					
				if(!in_array($character['wMapIndex'], $this->config->item('unstuck_no_move')))
				{
					...
					$this->template_data['template']['message'] = 'Your character was successfully moved!'; 						
				}
			} 			
