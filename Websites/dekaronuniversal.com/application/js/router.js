		...
		if(/logout/.test(link)){
			...
			$.get(link, { is_json_ajax: "1" }, function(data, textStatus, response){
				...
				if(Router.page == link){
					...
					UI.alert("Page load Error! Trying to load the page directly... <center style='margin-top:20px;'><img src='" + Config.URL + "application/images/modal-ajax.gif' /></center>", 3000);
					setTimeout(function(){
						window.location = link;
					}, 3000);
				}
