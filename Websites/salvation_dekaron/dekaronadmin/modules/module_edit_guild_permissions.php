...
if (isset($_POST) && !empty($_POST))
{
    ...
    {
		$POST = notice_message_admin('You have the following errors:<br>'.$validate->GetErrors().'', '0', '1', 'index.php?get=module_edit_guild_permissions&guild='.$_GET['guild'].'');
    }
    else
	{
		...
		$POST = notice_message_admin('Guild Permissions Names successfully updated', '1', '0', 'index.php?get=module_edit_guild_permissions&guild='.$_GET['guild'].'');
	}
}
else
{
            	...
            	<td align="center" style="padding-top: 20px; padding-bottom: 20px;">
                	<select class="medium" name="select1" onchange="location.href=navigation.select1.options[selectedIndex].value">';
                        ...
                        while ($getGuildInfo = $db->SQLfetchArray($SQLquery1))
                        {
                            ...
                            $data .= '<option value="index.php?get=module_edit_guild_permissions&guild='.$_GET['guild'].'&peerage_code='.htmlspecialchars($getGuildInfo['peerage_code']).'&peerage_name='.htmlspecialchars($getGuildInfo['peerage_name']).'">'.htmlspecialchars($getGuildInfo['peerage_name']).'</option>';	
                        }
                        $data .= '
            ...
            <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0"  class="table_panel" >
                ...
                <tr class="even">
                    <td width="45%" align="left" valign="top" class="panel_text_alt1"><b>Guild permissions</b>
                        <br>Set the permissions for <b>'.$_GET['peerage_name'].' (Level '.$_GET['peerage_code'].')</b>
                ...
                <tr>
                    <td align="right" class="panel_buttons" colspan="3"><input type="button" value="Refresh" onclick="ask_url("Are you sure you want to update?","index.php?get=module_edit_guild_permissions&guild=.'$_GET['guild'].'&peerage_code='.$_GET['peerage_code'].'&peerage_name='.$_GET['peerage_name'].'")"></td>
                </tr>
            </table>
            </form>';
