...
if (isset($_POST) && !empty($_POST))
{
    ...
    {
		$POST = notice_message_admin('You have the following errors:<br>'.$validate->GetErrors().'', '0', '1', 'index.php?get=module_edit_guild_permissions_names&guild='.$_GET['guild'].'');
    }
    else
	{
		...
		$POST = notice_message_admin('Guild Permissions Names successfully updated', '1', '0', 'index.php?get=module_edit_guild_permissions_names&guild='.$_GET['guild'].'');
	}
}
else
{
...
if($config->get('show_default', 'settings_edit_guild_permissions_names') == '1')
{
	...
	$smarty->assign("td", '<td align="left" class="panel_title_sub2">Default Names</td>');
}
else
{
	...
	$smarty->assign("guild", $_GET['guild']);
	...
	while ($getGuildInfo = $db->SQLfetchArray($SQLquery1))
	{
		...
		<tr class="' . $tr_color . '">
			...
			if($config->get('show_default', 'settings_edit_guild_permissions_names') == '1')
			{
				$table .= '<td align="left" class="panel_text_alt_list">'.htmlspecialchars($default_name[$getGuildInfo['peerage_code']]).'</td>';
			}
			else
			{
