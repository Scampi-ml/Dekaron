...
for($i = 0; $i < (int)$numSockets; ++$i)
{
	...
	else
	{
		...
		$socket[$i] = $optionDesc.'<span style="float:right;">('.$gemName.')</span>';
	}
}
...
$smarty->assign("special_color", $special_color);
$smarty->assign("special_name", $special_name);
$smarty->assign("new_item_name", $new_item_name);
...
{ 
	...
	{
		$socket_info_html .=  '<div style="margin-left:10px;  text-decoration:none">Weapon Type : '.$weapon_type[GetItemData($csv_file, $windex, 0, 3)].'</div>';
		$socket_info_html .=  '<br>';
	}
}


$socket_info_html .=  '<div style="margin-left:10px;  text-decoration:none">Class : '.$job[GetItemData($csv_file, $windex, 0, 8)].'</div>';

if(GetItemData($csv_file, $windex, 0, 13) != '0')
{
	$socket_info_html .=  '<br>';
	$socket_info_html .=  '<div style="margin-left:10px;  text-decoration:none">Selling Price : '.number_format(GetItemData($csv_file, $windex, 0, 13)).'</span></div>'; 
}
...
{
	$socket_info_html .=  '<br>';
	$socket_info_html .=  '<div style="margin-left:10px;  text-decoration:none">Defense : '.GetItemData($csv_file, $windex, 0, 66).'</div>';
}


if(GetItemData($csv_file, $windex, 0, 12) != '0')
{			
	$socket_info_html .=  '<div style="margin-left:10px;  text-decoration:none">Req. Level : Lv '.GetItemData($csv_file, $windex, 0, 12).'</div>';
}


if(GetItemData($csv_file, $windex, 0, 70) != '0')
{
	$socket_info_html .=  '<div style="margin-left:10px;  text-decoration:none">Req. Str : '.GetItemData($csv_file, $windex, 0, 70).'</div>';
}


if(GetItemData($csv_file, $windex, 0, 71) != '0')
{			
	$socket_info_html .=  '<div style="margin-left:10px;  text-decoration:none">Req. Dex : '.GetItemData($csv_file, $windex, 0, 71).'</div>';
}


if(GetItemData($csv_file, $windex, 0, 72) != '0')
{
	$socket_info_html .=  '<div style="margin-left:10px;  text-decoration:none">Req. Spr : '.GetItemData($csv_file, $windex, 0, 72).'</div>';
}


if(GetItemData($csv_file, $windex, 0, 62) != '0' && GetItemData($csv_file, $windex, 0, 63) != '0')
{
	$socket_info_html .=  '<div style="margin-left:10px;  text-decoration:none">Physical Damage : '.GetItemData($csv_file, $windex, 0, 62).' ~ '.GetItemData($csv_file, $windex, 0, 63).'</div>';
}	
		

if(GetItemData($csv_file, $windex, 0, 64) != '0' && GetItemData($csv_file, $windex, 0, 65) != '0')
{
	$socket_info_html .=  '<div style="margin-left:10px;  text-decoration:none">Magic Damage : '.GetItemData($csv_file, $windex, 0, 64).' ~ '.GetItemData($csv_file, $windex, 0, 65).'</div>';
}


if(GetItemData($csv_file, $windex, 0, 62) != '0' || GetItemData($csv_file, $windex, 0, 64) != '0')
{
	$socket_info_html .=  '<div style="margin-left:10px;  text-decoration:none">Attack Speed : 1</div>';
}


if(GetItemData($csv_file, $windex, 0, 68) != '0')
{
	$socket_info_html .=  '<div style="margin-left:10px;  text-decoration:none">Block Rate : '.GetItemData($csv_file, $windex, 0, 68).'%</div>';	
}



$smarty->assign("socket_info_html", $socket_info_html.'<br><br>');
$socket_html = "";
if($socket[0] != 'No Socket' )
{
	$socket_html .= '<div style="margin-left:10px; color: #F00; text-decoration:none">'.$socket[0].'</div>';
	
}

if($socket[1] != 'No Socket')
{
	$socket_html .= '<div style="margin-left:10px; color: #F00; text-decoration:none">'.$socket[1].'</div>';
	
}

if($socket[2] != 'No Socket')
{
	$socket_html .= '<div style="margin-left:10px; color: #F00; text-decoration:none">'.$socket[2].'</div>';
	
}

if($socket[3] != 'No Socket')
{
	$socket_html .= '<div style="margin-left:10px; color: #F00; text-decoration:none">'.$socket[3].'</div>';
	
}

$smarty->assign("socket_html", $socket_html);
// options
$option_html = "";
if($opt[0] != 'No Option')
{
$option_html .= '<div style="margin-left:10px; color: #6699cc; text-decoration:none">'.$opt[0].' ('.$optionDesc.')</div>';

}

if($opt[1] != 'No Option')
{
$option_html .= '<div style="margin-left:10px; color: #6699cc; text-decoration:none">'.$opt[1].' ('.$optionDesc.')</div>';

}

if($opt[2] != 'No Option')
{
$option_html .= '<div style="margin-left:10px; color: #ccccff; text-decoration:none">'.$opt[2].' ('.$optionDesc.')</div>';

}

if($opt[3] != 'No Option')
{
$option_html .= '<div style="margin-left:10px; color: #ffff00; text-decoration:none">'.$opt[3].' ('.$optionDesc.')</div>';

}

$smarty->assign("option_html", $option_html);
