...
class Ranking_level extends MY_Controller
{	
	...
	{
		$my_data = $this->cache->file->get('ranking_level');
		if($my_data === FALSE)
		{		
			...
			$this->cache->file->save('ranking_level', $result, $this->config->item('ranking_update_interval'));
		}
		else
		{
			$result =  $my_data;
		...
		$this->load->view('ranks/view_ranking_level', $this->template_data);
	...
	public function Force()
	{
		...
		else
		{
			...
			if($get_password == $this->config->item('ranking_force_update_password'))
			{
				...
				redirect('ranks/ranking_level', 'refresh'); 					
			}
			else
			{
