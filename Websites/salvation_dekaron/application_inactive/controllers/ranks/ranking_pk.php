...
class Ranking_pk extends MY_Controller
{	
	...
	{
		$my_data = $this->cache->file->get('ranking_pk');
		if($my_data === FALSE)
		{		
			...
			$this->cache->file->save('ranking_pk', $result, $this->config->item('ranking_update_interval'));
		}
		else
		{
			$result =  $my_data;
		...
		$this->load->view('ranks/view_ranking_pk', $this->template_data);
	...
	public function Force()
	{
		...
		else
		{
			...
			if($get_password == $this->config->item('ranking_force_update_password'))
			{
				...
				redirect('ranks/ranking_pk', 'refresh'); 					
			}
			else
			{
