...
class Validate extends MY_Controller
{
			...
			if (strcmp ($res, "VERIFIED") == 0 && $payment_status == 'Completed' && $receiver_email == $this->config->item('paypal_email')) 
			{
				...
				if($validpackage)
				{
					...
					if($account_check)
					{
						...
						{
							$this->dbg_log('ERROR SENDING COINS: '.$buff);
						}
					}
					else
					{
						$this->dbg_log('ACCOUNT NOT FOUND: '.$buff);
					}
				}
				else
				{
					$this->dbg_log('INVALID PACKAGE: '.$buff);
				}
			}
			elseif (strcmp ($res, "INVALID") == 0) 
			{
				// log for manual investigation
				$this->dbg_log('INVALID IPN: '.$buff);
			}
		}
	}
	...
	public function WritePaypalLog($msg)
	{
		...
		write_file('application/cache/hitcounters/'.$logfile, $write);
	}
